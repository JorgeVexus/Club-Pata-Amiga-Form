...
import PlanMembersReport from './PlanMembersReport';

export default function InteractiveReports() {
    const [analyticsData, setAnalyticsData] = useState<any>(null);
    const [loading, setLoading] = useState(true);
    const [range, setRange] = useState('30d');
    const [activeTab, setActiveTab] = useState<'analytics' | 'pata-amiga'>('analytics');
    ...
    return (
        <div className={styles.reportsContainer}>
            {/* Header dinámico por pestaña */}
            <div className={styles.reportsHeader}>
                <div className={styles.titleGroup}>
                    <h2 className={styles.reportsTitle}>
                        {activeTab === 'analytics' ? 'Gráficas e Inteligencia de Datos' : 'Reporte de Miembros (Plan Pata Amiga)'}
                    </h2>
                    <p className={styles.reportsSubtitle}>
                        {activeTab === 'analytics' 
                            ? 'Visualización en tiempo real de la salud del Club' 
                            : 'Lista detallada de suscriptores y estados de pago cruzados con la base de datos'}
                    </p>
                </div>
                {activeTab === 'analytics' && (
            ...
            <div className={styles.tabsContainer}>
                ...
                <button
                    className={`${styles.tabButton} ${activeTab === 'pata-amiga' ? styles.active : ''}`}
                    onClick={() => setActiveTab('pata-amiga')}
                >
                    👥 Miembros Club Pata Amiga
                </button>
            </div>
